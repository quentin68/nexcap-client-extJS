Ext.define('Dashboard.view.consultation.borrowings.MainController',{
    extend: 'Ext.app.ViewController',
    alias: 'controller.consultationBorrowingsMain',
    
    require:[],
    
    className: 'MainController',
    feature: Ext.create('Dashboard.store.Features').findRecord('name', 'CUR_BORROW_INVENTORY', 0, false, true, true),  
    ...
    init: function() {
        ...
        this.className = Ext.getClass(this).getName();
         ...
         this.control({
            ...
            'consultationBorrowingsMain viewListGrid': {itemclick: this.onSelectItem}
        ...
        try{
            Dashboard.model.operation.Borrowing.load(id, {
                ...
                success: function(record, operation) {
                    ...
                    console.log('borrowing: ' + record.get('material.name'));
    ...
    displayDetail: function(record){
        //Ext.ComponentQuery.query('borrowingsDetail')[0].setData(record.data);
    ...
    exportToExel: function () {
        ...
        var link;
        if (contextSelected) {
            link= '/inventories/borrowings/export?context='+ encodeURIComponent(contextSelected);
        }
        else {
            link = '/inventories/borrowings/export';
        }
        //var link = '/inventories/borrowings/export';
        var data = Dashboard.engine.ExportToFile.getExportParams(main);
        Dashboard.engine.ExportToFile.doRequest(link, data, sort, filter);
    },

    getMain: function(){
        return   Ext.ComponentQuery.query('consultationBorrowingsMain')[0];
    }
});