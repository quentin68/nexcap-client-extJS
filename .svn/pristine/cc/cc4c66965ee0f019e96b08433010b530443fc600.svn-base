...
Ext.define('Dashboard.manager.system.DevicesManager',{
    extend: 'Ext.app.Controller',
    alias: 'devicesManager',
    singleton: true,
    
    requires:[
        'Dashboard.tool.Utilities',
        'Dashboard.store.administration.ServerDevicesTypes',
        'Dashboard.store.system.DevicesTypes'
    ],
    
    store: Ext.create('Dashboard.store.system.Devices',{
        autoLoad: false,
        sorters: [{
                property: 'name',
                direction: 'ASC'
...
//    serverDevicesTypes : Ext.create('Dashboard.store.administration.ServerDevicesTypes',{
//        autoLoad: true,
//        listeners:{
//            scope: this,
...
//                    var type = this.devicesTypes.findRecord('name', record.data.value);
//                    if(type){
//                        type.data.label = record.data.label;
//                    }
//                }, this);
//            }
//        }
//    }),
    ...
    loadUserConfiguration: function(feature){
        ...
        try {
            ...
            Dashboard.model.UserConfiguration.load(key, {
                scope: this,
                failure: function(record, operation) {
                    Dashboard.tool.Utilities.info('[DevicesManager.loadUserConfiguration] error: loading failure');
                ...
                success: function(record, operation) {
                    try {
                        ...
                        Dashboard.tool.Utilities.info('[DevicesManager.loadUserConfiguration] loading success. Feature: ' + configuration.data.featureName);
                                                                        
                    } catch (ex) {
                        Dashboard.tool.Utilities.error('[DevicesManager.loadUserConfiguration] loading error: ' + ex);
                    }
            ...
            Dashboard.tool.Utilities.error('[DevicesManager.loadConfiguration] error: ' + err);
        }
    },
        ...
        try{
            Dashboard.model.system.Device.load(id, {
                ...
                success: function(record, operation) {
                    ...
                    }else if (action === 'setDeviceData') {
                        controller.setDeviceData(model);
                    }
                },
    ...
    update: function (model, controller, action, temp) {
        if (!model) {
            throw new Error('[DevicesManager.update] model is null or empty!');
        }
    ...
    requireLogs: function(deviceId, controller, action){
        ...
        Ext.Ajax.request({
            url: Dashboard.config.Config.SERVER_HOST_NAME + '/devices/requirelog',
            cors: true,
    ...
    shareConfiguration: function(device, controller, action, params){
        ...
        Ext.Ajax.request({
            url: Dashboard.config.Config.SERVER_HOST_NAME + '/devices/configuration/copy',
            cors: true,
            useDefaultXhrHeader: false,
            withCredentials: true,
    ...
    deleteOne: function(id, controller, action){
        if(!id){
            throw new Error('['+this.className+'.deleteOne] device is null or empty!');
        }

        var model = Ext.create('Dashboard.model.system.Device', {
            id:id
        });
    ...
    banDevice: function(id, controller, action){
        if(!id){
            throw new Error('['+this.className+'.banDevice] device is null or empty!');
        }

        Ext.Ajax.request({
            url: Dashboard.config.Config.SERVER_HOST_NAME + '/devices/unauthorize',
            cors: true,
            useDefaultXhrHeader: false,
            withCredentials: true,
