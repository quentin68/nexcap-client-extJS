Ext.define('Dashboard.manager.settings.alert.LocationsAlertsConfigManager', {
    extend: 'Ext.app.Controller',
    alias: 'locationsAlertsConfigManager',
    singleton: true,
    ...
    loadConfiguration: function(feature, isAdmin){
        ...
        Dashboard.manager.settings.alert.LocationsAlertsConfigManager.isAdmin = isAdmin;
        ...
        try{
            ...
            Dashboard.model.FeatureConfiguration.load(feature.data.name, {
                ...
                failure: function(record, operation) {
                    ...
                    Dashboard.tool.Utilities.info('[LocationsAlertsConfigManager.loadConfiguration] error: loading failure');
                ...
                success: function(record, operation) {                   
                    try {
                        ...
                        Dashboard.tool.Utilities.info('[LocationsAlertsConfigManager.loadConfiguration] loading success. Feature: ' + model.data.featureName);

                        this.loadUserConfiguration(feature);
                    } catch (ex) {
                        Dashboard.manager.FeaturesManager.deleteFeature(feature.data.name, 'LocationsAlertsConfigManager'); // 6843
                    }   
                }
            });
            
        }catch(err){
            Dashboard.tool.Utilities.error('[LocationsAlertsConfigManager.loadConfiguration] error: ' + err);
        }
    ...
    loadUserConfiguration: function(feature){
        ...
        try {
            ...
            Dashboard.model.UserConfiguration.load(key, {
                scope: this,
                failure: function(record, operation) {
                    Dashboard.tool.Utilities.info('[LocationsAlertsConfigManager.loadUserConfiguration] error: loading failure');
                ...
                success: function(record, operation) {
                    try {
                        ...
                        Dashboard.tool.Utilities.info('[LocationsAlertsConfigManager.loadUserConfiguration] loading success. Feature: ' + configuration.data.featureName);
                                                                        
                    } catch (ex) {
                        Dashboard.tool.Utilities.error('[LocationsAlertsConfigManager.loadUserConfiguration] loading error: ' + ex);
                    }
    ...
    getOne: function (id, controller, action) {
        ...
        try {
            Dashboard.model.settings.AlertsConfig.load(id, {
                scope: this,
                failure: function (record, operation) {
                    Dashboard.tool.Utilities.info('[LocationsAlertsConfigManager.getOne] error: loading failure');
                },
                success: function (record, operation) {
                    ...
                    Dashboard.tool.Utilities.info('[LocationsAlertsConfigManager.getOne] loading success. Profile: ' + model.data.name);
            ...
            Dashboard.tool.Utilities.error('[LocationsAlertsConfigManager.getOne] error: ' + err);
        }
  ...
  save: function(model, controller, action){
        if(!model){
            throw new Error('[LocationsAlertsConfigManager.save] model is null or empty!');
        }
        ...
        model.save({
            ...
            success: function(record, response) {
                ...
                Dashboard.tool.Utilities.info(
                        '[LocationsAlertsConfigManager.save] save and read success : name: '+ model.data.name);
...
//                    console.log(response);
    ...
    update: function(model, controller, action){
        if(!model){
            throw new Error('[LocationsAlertsConfigManager.update] model is null or empty!');
        }        
    ...
    deleteOne: function(id, controller, action){
        if(!id){
            throw new Error('[LocationsAlertsConfigManager.deleteOne] user is null or empty!');
        }
        ...
        model.erase({
            success: function() {
                ...
                Dashboard.tool.Utilities.info('[LocationsAlertsConfigManager.deleteOne] success: alert deleted');
    ...
    displayInMain: function(feature){
        ...
        Dashboard.tool.Utilities.info('[LocationsAlertsConfigManager.displayInMain] show server configuration Management feature');
...
//        this.store.getProxy().setActionMethods({read: 'POST'});
    ...
    getConfiguration: function(){
        ...
        var configuration = {
            ...
            enabledTools: {
                create: Dashboard.manager.settings.alert.LocationsAlertsConfigManager.isAdmin,
                edit: Dashboard.manager.settings.alert.LocationsAlertsConfigManager.isAdmin,
                read: false,
                destroy: Dashboard.manager.settings.alert.LocationsAlertsConfigManager.isAdmin,
                duplicate: false,
                exportToFile: false,
                toggle: true,
                configViewList: true
        ...
        if(Dashboard.manager.MainMenuManager.currentFeature === 'SYSTEM_ALERTS_LOCATIONS' ){
           
        }else if(Dashboard.manager.MainMenuManager.currentFeature === 'SETTINGS_ALERTS_LOCATIONS'){
