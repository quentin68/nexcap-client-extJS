...
Ext.define('Dashboard.manager.historic.alert.LocationsAlertsManager', {
    extend: 'Ext.app.Controller',
    alias: 'histoLocationsAlertsManager',
    singleton: true,

    requires: ['Dashboard.tool.Utilities'],

    store: Ext.create('Dashboard.store.historic.alerts.Locations', {
        sorters: [{
                property: 'startDate',
                direction: 'DESC'
            }],
    ...
    displayInMain: function (feature){
        ...
        mainController.displayInMain({
            ...
            tag: 'main',
            detailAlias: 'historicLocationAlertDetail'
        });
    },
    ...
    getOne: function (id, controller, action){
        ...
        try {
            Dashboard.model.historic.alerts.LocationAlert.load(id, {
                scope: this,
                failure: function (record, operation){
                    Dashboard.tool.Utilities.info('[LocationsAlertsManager.getOne] error: loading failure');
                },
                success: function (record, operation){

                    var response = Ext.decode(operation._response.responseText);
                    var model = Ext.create('Dashboard.model.alerts.LocationAlert', response.data);

                    Dashboard.tool.Utilities.info('[LocationsAlertsManager.getOne] loading success. Profile: ' + model.data.name);
            ...
            Dashboard.tool.Utilities.error('[LocationsAlertsManager.getOne] error: ' + err);
        }
