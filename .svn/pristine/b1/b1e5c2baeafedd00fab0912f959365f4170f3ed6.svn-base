Ext.define('Dashboard.manager.historic.ConsumptionsManager', {
    extend: 'Ext.app.Controller',
    alias: 'consumptionsManager',
    singleton: true,
    ...
    store: Ext.create('Dashboard.store.historic.Consumptions', {
        sorters: [{
        ...
        try {
            ...
            Dashboard.model.FeatureConfiguration.load(this.feature.data.name, {
                ...
                failure: function (record, operation){
                    Dashboard.tool.Utilities.info('[ConsumptionsManager.loadConfiguration] error: loading failure');
                    this.displayInMain();
                },
                success: function (record, operation){
                    try {
                        ...
                        Dashboard.tool.Utilities.info('[ConsumptionsManager.loadConfiguration] loading success. Feature: ' + model.data.featureName);

                        this.displayInMain();
                    } catch (ex) {
                        Dashboard.manager.FeaturesManager.deleteFeature(this.feature.data.name, 'ConsumptionsManager'); // 6843
                    }
                }
            });

        } catch (err) {
            Dashboard.tool.Utilities.error('[ConsumptionsManager.loadConfiguration] error: ' + err);
        }
    ...
    displayInMain: function (){
        ...
        Dashboard.tool.Utilities.info('[ConsumptionsManager.displayInMain] show consumptions Management feature');
        ...
        try {
            ...
            Dashboard.tool.Utilities.error('[ConsumptionsManager.displayInMain] configuration error: ' + ex);
        }
        if (!configuration) {
    ...
    getOne: function (id, controller, action){
        ...
        try {
            Dashboard.model.historic.Profile.load(id, {
                scope: this,
                failure: function (record, operation){
                    Dashboard.tool.Utilities.info('[ConsumptionsManager.getOne] error: loading failure');
                },
                success: function (record, operation){

                    var response = Ext.decode(operation._response.responseText);
                    var model = Ext.create('Dashboard.model.historic.Consumption', response.data);

                    Dashboard.tool.Utilities.info('[ConsumptionsManager.getOne] loading success. Profile: ' + model.data.name);
            ...
            Dashboard.tool.Utilities.error('[ConsumptionsManager.getOne] error: ' + err);
        }
