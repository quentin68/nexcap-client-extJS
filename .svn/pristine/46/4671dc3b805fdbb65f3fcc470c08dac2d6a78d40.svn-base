Ext.define('Dashboard.manager.settings.alert.StocksAlertsConfigManager', {
    extend: 'Ext.app.Controller',
    alias: 'stocksAlertsConfigManager',
    singleton: true,
    ...
    loadConfiguration: function(feature, isAdmin){
        ...
        Dashboard.manager.settings.alert.StocksAlertsConfigManager.isAdmin = isAdmin;
        ...
        try{
            ...
            Dashboard.model.FeatureConfiguration.load(feature.data.name, {
                ...
                failure: function(record, operation) {
                    ...
                    Dashboard.tool.Utilities.info('[StocksAlertsConfigManager.loadConfiguration] error: loading failure');
                ...
                success: function(record, operation) {                   
                    try {
                        ...
                        Dashboard.tool.Utilities.info('[StocksAlertsConfigManager.loadConfiguration] loading success. Feature: ' + model.data.featureName);

                        this.loadUserConfiguration(feature);
                    } catch (ex) {
                        Dashboard.manager.FeaturesManager.deleteFeature(feature.data.name, 'StocksAlertsConfigManager'); // 6843
                    }   
                }
            });
            
        }catch(err){
            Dashboard.tool.Utilities.error('[StocksAlertsConfigManager.loadConfiguration] error: ' + err);
        }
    ...
    loadUserConfiguration: function(feature){
        ...
        try {
            ...
            Dashboard.model.UserConfiguration.load(key, {
                scope: this,
                failure: function(record, operation) {
                    Dashboard.tool.Utilities.info('[StocksAlertsConfigManager.loadUserConfiguration] error: loading failure');
                ...
                success: function(record, operation) {
                    try {
                        ...
                        Dashboard.tool.Utilities.info('[StocksAlertsConfigManager.loadUserConfiguration] loading success. Feature: ' + configuration.data.featureName);
                                                                        
                    } catch (ex) {
                        Dashboard.tool.Utilities.error('[StocksAlertsConfigManager.loadUserConfiguration] loading error: ' + ex);
                    }
    ...
    getOne: function (id, controller, action) {
        ...
        try {
            Dashboard.model.settings.AlertsConfig.load(id, {
                scope: this,
                failure: function (record, operation) {
                    Dashboard.tool.Utilities.info('[StocksAlertsConfigManager.getOne] error: loading failure');
                },
                success: function (record, operation) {
                    ...
                    Dashboard.tool.Utilities.info('[StocksAlertsConfigManager.getOne] loading success. Profile: ' + model.data.name);
            ...
            Dashboard.tool.Utilities.error('[StocksAlertsConfigManager.getOne] error: ' + err);
        }
  ...
  save: function(model, controller, action){
        if(!model){
            throw new Error('[StocksAlertsConfigManager.save] model is null or empty!');
        }
        ...
        model.save({
            ...
            success: function(record, response) {
                ...
                Dashboard.tool.Utilities.info(
                        '[StocksAlertsConfigManager.save] save and read success : name: '+ model.data.name);
...
//                    console.log(response);
    ...
    update: function(model, controller, action){
        if(!model){
            throw new Error('[StocksAlertsConfigManager.update] model is null or empty!');
        }        
    ...
    deleteOne: function(id, controller, action){
        if(!id){
            throw new Error('[StocksAlertsConfigManager.deleteOne] user is null or empty!');
        }
        ...
        model.erase({
            success: function() {
                ...
                Dashboard.tool.Utilities.info('[StocksAlertsConfigManager.deleteOne] success: alert deleted');
    ...
    displayInMain: function(feature){
        ...
        Dashboard.tool.Utilities.info('[StocksAlertsConfigManager.displayInMain] show server configuration Management feature');
...
//        this.store.getProxy().setActionMethods({read: 'POST'});
    ...
    getConfiguration: function(){
        ...
        var configuration = {
            ...
            enabledTools: {
                create: Dashboard.manager.settings.alert.StocksAlertsConfigManager.isAdmin,
                edit: Dashboard.manager.settings.alert.StocksAlertsConfigManager.isAdmin,
                read: false,
                destroy: Dashboard.manager.settings.alert.StocksAlertsConfigManager.isAdmin,
                duplicate: false,
                exportToFile: false,
                toggle: true,
                configViewList: true
        ...
        if(Dashboard.manager.MainMenuManager.currentFeature === 'SYSTEM_ALERTS_STOCKS' ){
           
        }else if(Dashboard.manager.MainMenuManager.currentFeature === 'SETTINGS_ALERTS_STOCKS'){
