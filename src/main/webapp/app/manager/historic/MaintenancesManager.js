Ext.define('Dashboard.manager.historic.MaintenancesManager',{
    extend: 'Ext.app.Controller',
    alias: 'historicMaintenancesManager',
    singleton: true,
    ...
    store: Ext.create('Dashboard.store.historic.Maintenances',{
        sorters : [ {
                property : 'id',
                direction : 'DESC'
        } ],
            ...
            Dashboard.model.FeatureConfiguration.load(this.feature.data.name, {
                ...
                success: function(record, operation) {
                    try {
                        ...
                        Dashboard.manager.FeaturesManager.deleteFeature(this.feature.data.name, 'MaintenancesManager'); // 6843
                    }
                }
            });
    ...
    displayInMain: function(){
        ...
        Dashboard.tool.Utilities.info('[MaintenancesManager.displayInMain] show maintenances Management feature');
    ...
    getOne: function(id, controller, action){
        ...
        try{
            Dashboard.model.historic.Profile.load(id, {
                scope: this,
                failure: function(record, operation) {
                    Dashboard.tool.Utilities.info('[MaintenancesManager.getOne] error: loading failure');
                },
                success: function(record, operation) {
                    
                    var response = Ext.decode(operation._response.responseText);
                    var model = Ext.create('Dashboard.model.historic.Maintenance', response.data);
                    
                    Dashboard.tool.Utilities.info('[MaintenancesManager.getOne] loading success. Profile: '+ model.data.name);
