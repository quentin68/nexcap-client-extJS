...
Ext.define('Dashboard.manager.historic.alert.DevicesAlertsManager', {
    extend: 'Ext.app.Controller',
    alias: 'histoDevicesAlertsManager',
    singleton: true,

    requires: ['Dashboard.tool.Utilities'],

    store: Ext.create('Dashboard.store.historic.alerts.Devices', {
        sorters: [{
                property: 'startDate',
                direction: 'DESC'
            }],
        ...
        try {
            ...
            Dashboard.model.FeatureConfiguration.load(feature.data.name, {
                ...
                failure: function (record, operation){
                    Dashboard.tool.Utilities.info('[DevicesAlertsManager.loadConfiguration] error: loading failure');
                ...
                success: function (record, operation){
                    try {
                        ...
                        Dashboard.tool.Utilities.info('[DevicesAlertsManager.loadConfiguration] loading success. Feature: ' + model.data.featureName);

                        this.loadUserConfiguration(feature);
                    } catch (ex) {
                        Dashboard.manager.FeaturesManager.deleteFeature(feature.data.name, 'DevicesAlertsManager'); // 6843
                    }
                }
            });

        } catch (err) {
            Dashboard.tool.Utilities.error('[DevicesAlertsManager.loadConfiguration] error: ' + err);
        }
    ...
    loadUserConfiguration: function (feature){
        ...
        try {
            ...
            Dashboard.model.UserConfiguration.load(key, {
                scope: this,
                failure: function (record, operation){
                    Dashboard.tool.Utilities.info('[DevicesAlertsManager.loadUserConfiguration] error: loading failure');
                ...
                success: function (record, operation){
                    try {
                        ...
                        Dashboard.tool.Utilities.info('[DevicesAlertsManager.loadUserConfiguration] loading success. Feature: ' + configuration.data.featureName);

                    } catch (ex) {
                        Dashboard.tool.Utilities.error('[DevicesAlertsManager.loadUserConfiguration] loading error: ' + ex);
                    }
    ...
    displayInMain: function (feature){
        ...
        Dashboard.tool.Utilities.info('[DevicesAlertsManager.displayInMain] show alerts Management feature');
        ...
        try {
            ...
            Dashboard.tool.Utilities.error('[DevicesAlertsManager.displayInMain] configuration error: ' + ex);
        }
        if (!configuration) {
        ...
        mainController.displayInMain({
            ...
            tag: 'main',
            detailAlias: 'historicDeviceAlertDetail'
        });
    },
    ...
    getOne: function (id, controller, action){
        ...
        try {
            Dashboard.model.historic.alerts.DeviceAlert.load(id, {
                scope: this,
                failure: function (record, operation){
                    Dashboard.tool.Utilities.info('[DevicesAlertsManager.getOne] error: loading failure');
                },
                success: function (record, operation){

                    var response = Ext.decode(operation._response.responseText);
                    var model = Ext.create('Dashboard.model.alerts.DeviceAlert', response.data);

                    Dashboard.tool.Utilities.info('[DevicesAlertsManager.getOne] loading success. Profile: ' + model.data.name);
            ...
            Dashboard.tool.Utilities.error('[DevicesAlertsManager.getOne] error: ' + err);
        }
