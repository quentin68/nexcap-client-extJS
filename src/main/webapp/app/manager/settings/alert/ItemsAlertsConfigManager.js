Ext.define('Dashboard.manager.settings.alert.ItemsAlertsConfigManager', {
    extend: 'Ext.app.Controller',
    alias: 'itemsAlertsConfigManager',
    singleton: true,
    ...
    loadConfiguration: function(feature, isAdmin){
        ...
        Dashboard.manager.settings.alert.ItemsAlertsConfigManager.isAdmin = isAdmin;
        ...
        try{
            ...
            Dashboard.model.FeatureConfiguration.load(feature.data.name, {
                ...
                failure: function(record, operation) {
                    ...
                    Dashboard.tool.Utilities.info('[ItemsAlertsConfigManager.loadConfiguration] error: loading failure');
                ...
                success: function(record, operation) {                   
                    try {
                        ...
                        Dashboard.tool.Utilities.info('[ItemsAlertsConfigManager.loadConfiguration] loading success. Feature: ' + model.data.featureName);

                        this.loadUserConfiguration(feature);
                    } catch (ex) {
                        Dashboard.manager.FeaturesManager.deleteFeature(feature.data.name, 'ItemsAlertsConfigManager'); // 6843
                    }   
                }
            });
            
        }catch(err){
            Dashboard.tool.Utilities.error('[ItemsAlertsConfigManager.loadConfiguration] error: ' + err);
        }
    ...
    loadUserConfiguration: function(feature){
        ...
        try {
            ...
            Dashboard.model.UserConfiguration.load(key, {
                scope: this,
                failure: function(record, operation) {
                    Dashboard.tool.Utilities.info('[ItemsAlertsConfigManager.loadUserConfiguration] error: loading failure');
                ...
                success: function(record, operation) {
                    try {
                        ...
                        Dashboard.tool.Utilities.info('[ItemsAlertsConfigManager.loadUserConfiguration] loading success. Feature: ' + configuration.data.featureName);
                                                                        
                    } catch (ex) {
                        Dashboard.tool.Utilities.error('[ItemsAlertsConfigManager.loadUserConfiguration] loading error: ' + ex);
                    }
    ...
    getOne: function (id, controller, action) {
        ...
        try {
            Dashboard.model.settings.AlertsConfig.load(id, {
                scope: this,
                failure: function (record, operation) {
                    Dashboard.tool.Utilities.info('[ItemsAlertsConfigManager.getOne] error: loading failure');
                },
                success: function (record, operation) {
                    ...
                    Dashboard.tool.Utilities.info('[ItemsAlertsConfigManager.getOne] loading success. Profile: ' + model.data.name);
  ...
  save: function(model, controller, action){
        if(!model){
            throw new Error('[ItemsAlertsConfigManager.save] model is null or empty!');
        }
        ...
        model.save({
            ...
            success: function(record, response) {
                ...
                Dashboard.tool.Utilities.info(
                        '[ItemsAlertsConfigManager.save] save and read success : name: '+ model.data.name);
...
//                    console.log(response);
    ...
    update: function(model, controller, action){
        if(!model){
            throw new Error('[ItemsAlertsConfigManager.update] model is null or empty!');
        }        
    ...
    deleteOne: function(id, controller, action){
        if(!id){
            throw new Error('[ItemsAlertsConfigManager.deleteOne] user is null or empty!');
        }
        ...
        model.erase({
            success: function() {
                ...
                Dashboard.tool.Utilities.info('[ItemsAlertsConfigManager.deleteOne] success: alert deleted');
    ...
    displayInMain: function(feature){
        ...
        Dashboard.tool.Utilities.info('[ItemsAlertsConfigManager.displayInMain] show server configuration Management feature');
...
//        this.store.getProxy().setActionMethods({read: 'POST'});
    ...
    getConfiguration: function(){
        ...
        var configuration = {
            ...
            enabledTools: {
                create: Dashboard.manager.settings.alert.ItemsAlertsConfigManager.isAdmin,
                edit: Dashboard.manager.settings.alert.ItemsAlertsConfigManager.isAdmin,
                read: false,
                destroy: Dashboard.manager.settings.alert.ItemsAlertsConfigManager.isAdmin,
                duplicate: false,
                exportToFile: false,
                toggle: true,
                configViewList: true
        ...
        if(Dashboard.manager.MainMenuManager.currentFeature === 'SYSTEM_ALERTS_ITEMS' ){
           
        }else if(Dashboard.manager.MainMenuManager.currentFeature === 'SETTINGS_ALERTS_ITEMS'){
