Ext.define('Dashboard.manager.settings.alert.DevicesAlertsConfigManager', {
    extend: 'Ext.app.Controller',
    alias: 'devicesAlertsConfigManager',
    singleton: true,
    ...
    loadConfiguration: function(feature, isAdmin){
        ...
        Dashboard.manager.settings.alert.DevicesAlertsConfigManager.isAdmin = isAdmin;
        ...
        try{
            ...
            Dashboard.model.FeatureConfiguration.load(feature.data.name, {
                ...
                failure: function(record, operation) {
                    ...
                    Dashboard.tool.Utilities.info('[DevicesAlertsConfigManager.loadConfiguration] error: loading failure');
                ...
                success: function(record, operation) {                   
                    try {
                        ...
                        Dashboard.tool.Utilities.info('[DevicesAlertsConfigManager.loadConfiguration] loading success. Feature: ' + model.data.featureName);

                        this.loadUserConfiguration(feature);
                    } catch (ex) {
                        Dashboard.manager.FeaturesManager.deleteFeature(feature.data.name, 'DevicesAlertsConfigManager'); // 6843
                    }   
                }
            });
            
        }catch(err){
            Dashboard.tool.Utilities.error('[DevicesAlertsConfigManager.loadConfiguration] error: ' + err);
        }
    ...
    loadUserConfiguration: function(feature){
        ...
        try {
            ...
            Dashboard.model.UserConfiguration.load(key, {
                scope: this,
                failure: function(record, operation) {
                    Dashboard.tool.Utilities.info('[DevicesAlertsConfigManager.loadUserConfiguration] error: loading failure');
                ...
                success: function(record, operation) {
                    try {
                        ...
                        Dashboard.tool.Utilities.info('[DevicesAlertsConfigManager.loadUserConfiguration] loading success. Feature: ' + configuration.data.featureName);
                                                                        
                    } catch (ex) {
                        Dashboard.tool.Utilities.error('[DevicesAlertsConfigManager.loadUserConfiguration] loading error: ' + ex);
                    }
    ...
    getOne: function (id, controller, action) {
        ...
        try {
            Dashboard.model.settings.AlertsConfig.load(id, {
                scope: this,
                failure: function (record, operation) {
                    Dashboard.tool.Utilities.info('[DevicesAlertsConfigManager.getOne] error: loading failure');
                },
                success: function (record, operation) {
                    ...
                    Dashboard.tool.Utilities.info('[DevicesAlertsConfigManager.getOne] loading success. Profile: ' + model.data.name);
            ...
            Dashboard.tool.Utilities.error('[DevicesAlertsConfigManager.getOne] error: ' + err);
        }
  ...
  save: function(model, controller, action){
        if(!model){
            throw new Error('[DevicesAlertsConfigManager.save] model is null or empty!');
        }
        ...
        model.save({
            ...
            success: function(record, response) {
                ...
                Dashboard.tool.Utilities.info(
                        '[DevicesAlertsConfigManager.save] save and read success : name: '+ model.data.name);
...
//                    console.log(response);
    ...
    update: function(model, controller, action){
        if(!model){
            throw new Error('[DevicesAlertsConfigManager.update] model is null or empty!');
        }        
    ...
    deleteOne: function(id, controller, action){
        if(!id){
            throw new Error('[DevicesAlertsConfigManager.deleteOne] user is null or empty!');
        }
        ...
        model.erase({
            success: function() {
                ...
                Dashboard.tool.Utilities.info('[DevicesAlertsConfigManager.deleteOne] success: alert deleted');
    ...
    displayInMain: function(feature){
        ...
        Dashboard.tool.Utilities.info('[DevicesAlertsConfigManager.displayInMain] show server configuration Management feature');
        ...
        mainController.displayInMain(
            { 
                ...
                title: getText('Devices alerts', 'title'),
                store: this.store,
    ...
    getConfiguration: function(){
        ...
        var configuration = {
            ...
            enabledTools: {
                create: Dashboard.manager.settings.alert.DevicesAlertsConfigManager.isAdmin,
                edit: Dashboard.manager.settings.alert.DevicesAlertsConfigManager.isAdmin,
                read: false,
                destroy: Dashboard.manager.settings.alert.DevicesAlertsConfigManager.isAdmin,
                duplicate: false,
                exportToFile: false,
                toggle: true,
                configViewList: true
        ...
        if(Dashboard.manager.MainMenuManager.currentFeature === 'SYSTEM_ALERTS_DEVICES' ){
           
        }else if(Dashboard.manager.MainMenuManager.currentFeature === 'SETTINGS_ALERTS_DEVICES'){
