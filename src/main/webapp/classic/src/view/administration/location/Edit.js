...
Ext.define('Dashboard.view.administration.location.Edit', {
    ...
    requires: [
        ...
        'Dashboard.view.administration.location.UsersGridPanel'
    ],
    ...
    initComponent: function(){
        ...
        this.title = getText('Edit a location');
...
//        };
    ...
    listeners: {
        ...
        afterrender: function (me, layout, eOpts) {
            ...
            me.onUserFeatureChangeEdit();
        }
    },
    onUserFeatureChangeEdit: function () {
    ...
    setData: function(data){
        ...
        this.down('combo[name=parentPositionId]').setValue(data.parentPosition.id);
        
        var codeField = this.down('textfield[name=code]');
        if (data.code && data.code.code) {
            var label = Dashboard.store.enums.CodeType[data.code.codeType +''].label;
            
            codeField.setFieldLabel(label);
            codeField.setValue(data.code.code);
            codeField.data = data.code;
        }

        this.usersStore.clearData();
        if (data.authorizedUsers) {
            for (var i = 0; i < data.authorizedUsers.length; i++) {
                this.usersStore.add(data.authorizedUsers[i]);
            }
        }
    },
    ...
    setThumbnail: function(tumb){
        ...
        var img = Ext.ComponentQuery.query('locationEdit image[name=thumbnailToEdit]')[0];
