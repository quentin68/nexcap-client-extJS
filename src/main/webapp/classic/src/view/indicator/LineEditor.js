...
Ext.define('Dashboard.view.indicator.LineEditor', {
    ...
    initComponent: function() {
        ...
        this.title = getText('Line editor');
...
//        };
            ...
            items:[
                {
                    ...
                    xtype: 'checkbox',
                    ui: 'lineEditorCheckbox',
                    fieldLabel: getText('Filling'),
                    boxLabel: getText('Filled', 'chart'),
                    name: 'filled',
                    reference: 'filled',
                    inputValue: true
                }
    ...
    getPropertySelector: function(color, label, name, smooth, marker, correlationId){
            ...
            items:[
                {
                    ...
                    xtype: 'checkbox',
                    boxLabel: getText('Smooth'),
                    name: 'smooth',
                    ui: 'lineEditorCheckbox',
                    inputValue: true,
                    checked: smooth ? smooth: false,
                    flex:0
                }, {
                    xtype: 'checkbox',
                    boxLabel: getText('Markers'),
                    name: 'marker',
                    ui: 'lineEditorCheckbox',
                    inputValue: true,
                    checked: marker ? marker: false,
                    flex:0
                }, {
                    xtype: 'button',
    ...
    */
    ...
    addPropertySelector: function(color, label, name, smooth, marker){
        
        var selector = this.getPropertySelector(color, label, name, smooth, marker);
        this.down('form').down('panel[reference=propertiesSelectors]').add(selector);
    },
    ...
    buildCalculByList: function(record){
        ...
        //init
        this.calculByList = this.getStatisticCalculByTypesList(record);
        Dashboard.tool.Utilities.debug('[LineEditor.buildCalculByList] init calculByList: '+ Ext.encode(this.calculByList));
        
        for(var i=0; i<statisticsCombos.length; i++){
            
            if(statisticsCombos[i].getValue()){
                Dashboard.tool.Utilities.debug('[LineEditor.buildCalculByList] statisticsCombos[i].getValue(): '+ statisticsCombos[i].getValue());
                ...
                Dashboard.tool.Utilities.debug('[LineEditor.buildCalculByList] newList: '+ Ext.encode(newList));
                ...
                for(var j=0; j<this.calculByList.length; j++){ 
                    ...
                    if(!exists){
                        Dashboard.tool.Utilities.debug('[LineEditor.buildCalculByList] delete calculBy: ' + this.calculByList[j].calculByType);
        ...
        Dashboard.tool.Utilities.info('[LineEditor.buildCalculByList] this.calculByList: '+ Ext.encode(this.calculByList));
    ...
    buildFilters: function(){
        ...
        Dashboard.tool.Utilities.info('[LineEditor.buildFilters] filters: '+ Ext.encode(filtersNamesList));
    ...
    displayFilters: function(filters){
        ...
        Dashboard.tool.Utilities.info('[LineEditor.displayFilters] filters: '+ Ext.encode(filters));
    ...
    getData: function(){
        ...
        if(values.filled !== undefined){
            data.dataBinding.filled = values.filled;
        }
        ...
        Ext.each(selectors, function(selector){
            ...
            if(statisticName !== undefined && statisticName){
                ...
                var dataProperty = {
                    ...
                    smooth: selector.down('field[name=smooth]').getValue(),
                    marker: selector.down('field[name=marker]').getValue(),
